...
@dataclass
class NemotronDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        tag_mapping = {
            self.dataset_attr.user_tag: Role.USER.value,
            self.dataset_attr.assistant_tag: Role.ASSISTANT.value,
            ...
            self.dataset_attr.function_tag: Role.FUNCTION.value,
            self.dataset_attr.system_tag: Role.SYSTEM.value,
            self.dataset_attr.pt_separator_tag: Role.PT_SEPARATOR.value,
            self.dataset_attr.pt_content_tag: Role.PT_CONTENT.value,
        }
        thinking_prompt = self.dataset_attr.thinking_prompt
        ...
        tool_responses = []
        broken_data = False
        has_reasoning = False # track whether message has reasoning_content

        for turn_idx, message in enumerate(messages):
            role = message[self.dataset_attr.role_tag]
            content = message[self.dataset_attr.content_tag]

            # Handle case where content is a dict (some datasets have this)
            if not isinstance(content, str):
                content = json.dumps(content, ensure_ascii=False)

            # Handle reasoning_content field (e.g., Nemotron, OpenAI reasoning models)
            reasoning_content = message.get("reasoning_content") or ""
            if reasoning_content:
                has_reasoning = True
                if thinking_prompt not in ("off", "none"):
                    content = f"<think>\n{reasoning_content}\n</think>\n\n{content}"
        ...
        odd_tags = (Role.USER.value, Role.OBSERVATION.value, Role.PT_SEPARATOR.value)
        even_tags = (Role.ASSISTANT.value, Role.FUNCTION.value, Role.PT_CONTENT.value)
        accept_tags = (odd_tags, even_tags)
        for turn_idx, message in enumerate(aligned_messages):
            if message["role"] not in accept_tags[turn_idx % 2]:
                logger.warning_rank0(f"Invalid role tag in {messages}.")
        ...
        # Determine thinking mode based on config or auto-detect
        if thinking_prompt == "on":
            if not has_reasoning:
                logger.warning_rank0('thinking_prompt set to on in dataset_info.json but NO reasoning detected in dataset!')
            default_system_prompt = "detailed thinking on"
        elif thinking_prompt == "off":
            if has_reasoning:
                logger.warning_rank0('thinking_prompt set to off in dataset_info.json but reasoning detected in dataset!')
            default_system_prompt = "detailed thinking off"
        elif thinking_prompt == "auto":  # "auto" or None - auto-detect based on content
            logger.warning_rank0_once('thinking_prompt set to auto-detect in dataset_info.json. This will set system prompt automatically based on reasoning content in the dataset.')
            default_system_prompt = "detailed thinking on" if has_reasoning else "detailed thinking off"
        elif thinking_prompt in ("none", "passthrough"): # no system prompt added
            default_system_prompt = ""
        else:
            raise ValueError(f"Thinking mode not valid: {thinking_prompt}")

        if default_system_prompt:
            if not system:
                system = default_system_prompt
            elif "detailed thinking on" not in system and "detailed thinking off" not in system:
                system += "\n" + default_system_prompt
...
DATASET_CONVERTERS = {
    ...
    'nemotron': NemotronDatasetConverter,
}
