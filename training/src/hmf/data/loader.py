...
from typing import TYPE_CHECKING, Literal, Optional, Union, Tuple, Callable, Any
...
from .data_utils import get_dataset_module, merge_dataset, read_cloud_json, split_dataset, load_jsonl
...
def _load_single_dataset(
    ...
    if dataset_attr.load_from == "ms_hub":
        ...
        if dataset_attr.load_from == "file" and "json" in data_path and dataset_attr.formatting in ("nemotron", "openai"):
            # TODO: Only nemotron and openai formats are supported for JSONL loading
            logger.info_rank0(f"Special loading of JSONL with tools: {data_files}")
            dataset = load_jsonl(data_files[0], dataset_attr, cache_dir=model_args.cache_dir)
        else:
    ...
    # align to common llamafactory format
    dataset = align_dataset(dataset, dataset_attr, data_args, training_args)

    # optionally apply additional filters
    if data_args.filter:
        dataset = _filter_aligned_dataset(dataset, dataset_attr, data_args, training_args)

    return dataset


def _filter_aligned_dataset(
    dataset: Union["Dataset", "IterableDataset"],
    dataset_attr: "DatasetAttr",
    data_args: "DataArguments",
    training_args: "Seq2SeqTrainingArguments",
) -> Union["Dataset", "IterableDataset"]:
    r"""Filter the aligned dataset based on the specified filter."""
    from .filter import get_dataset_filter

    assert not dataset_attr.ranking, "Filtering is not supported for ranking datasets"
    assert not dataset_attr.kto_tag, "Filtering is not supported for KTO datasets"

    # returns true on examples to be filtered
    filter_fn = get_dataset_filter(data_args.filter, dataset_attr, data_args)

    def should_keep(example: dict[str, Any]) -> bool:
        aligned_messages = example["_prompt"] + example["_response"]
        return not filter_fn(aligned_messages)

    original_len = len(dataset) if not data_args.streaming else None
    ...
    if not data_args.streaming:
        kwargs = dict(
            ...
            desc=f"Filtering dataset {dataset_attr.dataset_name}",
        )

    dataset = dataset.filter(should_keep, **kwargs)

    if not data_args.streaming:
        filtered_len = len(dataset)
        removed = original_len - filtered_len
        logger.info_rank0(
            f"Filtered {dataset_attr.dataset_name}: {original_len} -> {filtered_len} "
    ...
    with training_args.main_process_first(desc="pre-process dataset", local=(not data_args.data_shared_file_system)):
        ...
        if "train" in train_dict:
            ...
            if model_args.sequence_parallel_size > 1:
                train_dict["train"] = _get_sequence_parallel_dataset(
                    train_dict["train"], data_args, model_args, training_args, tokenizer, is_eval=False
                )
