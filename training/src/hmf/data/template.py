...
class Template:
    ...
    def get_ollama_modelfile(self, tokenizer: "PreTrainedTokenizer") -> str:
        ...
        """
        modelfile = "# ollama modelfile auto-generated by hybrid-model-factory\n\n"
...
@dataclass
class LongReasoningTemplate(ReasoningTemplate):
    """A ReasoningTemplate that supports PT_SEPARATOR and PT_CONTENT roles."""
    format_pt_separator: "Formatter"
    format_pt_content: "Formatter"

    def _is_pt_only(self, messages: list[dict[str, str]]) -> bool:
        """Check if all messages are pretraining roles."""
        pt_roles = {Role.PT_SEPARATOR, Role.PT_CONTENT}
        return all(message["role"] in pt_roles for message in messages)
    ...
    def encode_oneturn(
        ...
        # For PT-only data, skip ReasoningTemplate logic entirely
        if self._is_pt_only(messages):
            encoded_messages = self._encode(tokenizer, messages, system, tools)
            prompt_ids = []
            for encoded_ids in encoded_messages[:-1]:
                prompt_ids += encoded_ids
        ...
        # Otherwise use parent ReasoningTemplate behavior
        return super().encode_oneturn(tokenizer, messages, system, tools)
    ...
    def encode_multiturn(
        ...
        # Otherwise use parent ReasoningTemplate behavior
        return super().encode_multiturn(tokenizer, messages, system, tools)
    ...
    def _encode(
        ...
        # Check if this is a PT-only conversation
        is_pt_only = self._is_pt_only(messages)
        
        for i, message in enumerate(messages):
            elements = []

            if i == 0:
                # Skip prefix and system for PT-only data
                if not is_pt_only:
                    elements += self.format_prefix.apply()
                    if system or tools:
                        tool_text = self.format_tools.apply(content=tools)[0] if tools else ""
            ...
            elif message["role"] == Role.PT_SEPARATOR:
                elements += self.format_pt_separator.apply(content=message["content"])
            elif message["role"] == Role.PT_CONTENT:
                elements += self.format_pt_content.apply(content=message["content"])
            else:
                raise NotImplementedError("Unexpected role: {}".format(message["role"]))
...
def register_template(
    ...
    format_pt_separator: Optional["Formatter"] = None,
    format_pt_content: Optional["Formatter"] = None,
    default_system: str = "",
    ...
    if template_class in (LongReasoningTemplate,):
        base_kwargs["format_pt_separator"] = format_pt_separator or EmptyFormatter()
        base_kwargs["format_pt_content"] = format_pt_content or EmptyFormatter()

    TEMPLATES[name] = template_class(**base_kwargs)
...
register_template(
    ...
    format_tools=ToolFormatter(tool_format="qwen"),
    format_pt_separator=StringFormatter(slots=["<|endoftext|>{{content}}"]),
    format_pt_content=StringFormatter(slots=["{{content}}"]),
    stop_words=["<|im_end|>"],
    replace_eos=True,
    template_class=LongReasoningTemplate,
)
...
# qwen3 coder next uses a different XML-based tool format
register_template(
    name="qwen3_coder_next",
    format_user=StringFormatter(slots=["<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n"]),
    format_assistant=StringFormatter(slots=["{{content}}<|im_end|>\n"]),
    format_system=StringFormatter(slots=["<|im_start|>system\n{{content}}<|im_end|>\n"]),
    format_function=FunctionFormatter(slots=["{{content}}<|im_end|>\n"], tool_format="qwen3_coder_next"),
    format_observation=StringFormatter(
        slots=["<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n"]
    ),
    format_tools=ToolFormatter(tool_format="qwen3_coder_next"),
    stop_words=["<|im_end|>"],
    replace_eos=True,
)
...
register_template(
    name="qwen3_5_nothink_text_only",
    format_user=StringFormatter(slots=["<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n"]),
    format_assistant=StringFormatter(slots=["{{content}}<|im_end|>\n"]),
    format_system=StringFormatter(slots=["<|im_start|>system\n{{content}}<|im_end|>\n"]),
    format_function=FunctionFormatter(slots=["{{content}}<|im_end|>\n"], tool_format="qwen3_5"),
...
register_template(
    name="qwen3_5_nothink_text_only_pt",
    format_user=StringFormatter(slots=["<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n"]),
    format_assistant=StringFormatter(slots=["{{content}}<|im_end|>\n"]),
    format_system=StringFormatter(slots=["<|im_start|>system\n{{content}}<|im_end|>\n"]),
    format_function=FunctionFormatter(slots=["{{content}}<|im_end|>\n"], tool_format="qwen3_5"),
