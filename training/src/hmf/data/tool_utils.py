...
class Qwen3CoderNextToolUtils(ToolUtils):
    r"""Qwen3-Coder-Next tool using template."""
    ...
    def tool_formatter(tools: list[dict[str, Any]]) -> str:
        ...
        for tool in tools:
            ...
            if params.get("properties"):
                for pname, pfields in params["properties"].items():
                    ...
                    if pfields.get("description"):
                        tool_text += f"\n<description>{pfields['description'].strip()}</description>"
                    # Render extra keys (enum, items, etc.)
                    for k, v in pfields.items():
                        if k not in ("name", "type", "description"):
                            ...
                            tool_text += f"\n<{k}>{v_str}</{k}>"
                    tool_text += "\n</parameter>"
            # Render extra parameter-level keys (required, etc.)
            for k, v in params.items():
                if k not in ("type", "properties"):
                    v_str = v if isinstance(v, str) else json.dumps(v, ensure_ascii=False)
                    tool_text += f"\n<{k}>{v_str}</{k}>"
            tool_text += "\n</parameters>"
            # Render extra function-level keys
        ...
        return QWEN3_CODER_NEXT_TOOL_PROMPT.format(tool_text=tool_text)
    ...
    def function_formatter(functions: list["FunctionCall"]) -> str:
        ...
        for name, arguments in functions:
            ...
            if isinstance(args, dict):
                ...
                # Fallback: serialize entire args as single parameter
                text += f"<parameter=args>\n{json.dumps(args, ensure_ascii=False)}\n</parameter>\n"
            text += "</function>\n</tool_call>"
            parts.append(text)
        return "\n".join(parts)
    ...
    @staticmethod
    def tool_extractor(content: str) -> Union[str, list["FunctionCall"]]:
        regex = re.compile(r"<tool_call>\s*<function=([^>]+)>(.*?)</function>\s*</tool_call>", re.DOTALL)
        ...
        for func_name, params_block in matches:
            param_regex = re.compile(r"<parameter=([^>]+)>\s*(.*?)\s*</parameter>", re.DOTALL)
            param_matches = param_regex.findall(params_block)
            arguments = {}
            ...
            results.append(FunctionCall(func_name.strip(), json.dumps(arguments, ensure_ascii=False)))
...
TOOLS = {
    ...
    "qwen": QwenToolUtils(),
    "qwen3_coder_next": Qwen3CoderNextToolUtils(),
