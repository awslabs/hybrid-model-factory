...
@dataclass
class HybridizationArguments:
    r"""Arguments pertaining to model hybridization."""

    hybrid_learnable_params: Optional[str] = field(
        default=None,
        metadata={
            "help": "Comma-seprated strings that specify the learnable parameters in the hybrid model, e.g., 'mamba,self_attn'."
        },
    )
    
    hybrid_lw_distill_target: Optional[str] = field(
        default="none",
        metadata={
            "help": "Which outputs to align for MSE distillation. Options are 'decoder', 'mixer', 'residuals', and 'all'."
        },
    )
    
    hybrid_distill_pre_lm_head: Optional[bool] = field(
        default=True,
        metadata={
            "help": "Whether to apply the MSE distillation loss on the final hidden states just before the LM head."
        },
    )
...
class FinetuningArguments(
    ...
    save_max_shard_size: str = field(
        default="5GB",
        metadata={"help": "Maximum shard size when saving model checkpoints (e.g., '5GB', '10GB')."},
    )
    ...
    def __post_init__(self):
        ...
        if self.stage == "hkd":
            if self.hybrid_lw_distill_target == "none" and not self.hybrid_distill_pre_lm_head:
                raise ValueError(
                    "No distillation loss configured: `hybrid_lw_distill_target` is 'none' and "
                    "`hybrid_distill_pre_lm_head` is False. At least one must be enabled."
                )
