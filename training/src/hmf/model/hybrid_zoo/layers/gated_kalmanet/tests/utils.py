...
def init_sp_distr():
    ...
    if rank == 0:
        print(f"Remark: we are using CP = {world_size}")
    cp_size = world_size
...
def get_local_result(
    ...
    local_model = local_model(config=config, layer_idx=0)
    ...
    # Forward
    local_out = local_model(local_input_tensor)[0]
...
def get_global_result(
    ...
    global_model = global_model(config=config, layer_idx=0)

    global_model.load_state_dict(state_dict)
    global_model.to(device, dtype)

    # Forward
    global_out = global_model(input_tensor)[0]
...
def get_argparser():
    parser = argparse.ArgumentParser(description="ZigZag‑GKA distributed sanity‑check")

    parser.add_argument("--exp_name", type=str, default="gka", help="Experiment name")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to state_dict for loading GKA parameters",
    )
    parser.add_argument(
        "--sp",
        type=int,
...
def logs_to_json(args, logs, exp_name="GKA"):
    record = {
