...
def get_local_result(
    ...
    local_model = local_model(
        d_model=hidden_dim,
        d_xb=1024,  # d_xb = kv heads * head_dim
        d_inner=hidden_dim,  # d_inner = num attention heads * head_dim
        ngroups=hidden_dim // head_dim,
        d_state=128,
        qk_norm=Qwen3MoeRMSNorm,
        expand=1,
    )
    local_model.use_qk_norm = True
    local_model.use_pos_emb = False

    pos_embb = None
    if local_model.use_pos_emb:
        position_ids = (
            torch.arange(seqlen, device=device, dtype=torch.long)
            .unsqueeze(0)
            .repeat(batch_size, 1)
        )
        local_model.apply_rotary_pos_emb_fn = apply_rotary_pos_emb
    ...
    if local_model.use_pos_emb:
        local_position_ids = (
            extract_local(position_ids, rank, world_size).detach().clone()
        )
        rotator_emb = Qwen2RotaryEmbedding(config=config)
        cos, sin = rotator_emb(local_input_tensor, local_position_ids)
        pos_embb = (cos, sin)
    ...
    # Forward
    local_out = local_model(local_input_tensor, pos_embb)[0]
...
def get_global_result(
    ...
    global_model.use_qk_norm = True
    global_model.use_pos_emb = False
    pos_embb = None

    global_model.load_state_dict(state_dict)
    global_model.to(device, dtype)

    if global_model.use_pos_emb:
        position_ids = (
            torch.arange(seqlen, device=device, dtype=torch.long)
            .unsqueeze(0)
            .repeat(batch_size, 1)
        ...
        rotator_emb = Qwen2RotaryEmbedding(config=config)
        cos, sin = rotator_emb(input_tensor, position_ids)
        pos_embb = (cos, sin)
        global_model.apply_rotary_pos_emb_fn = apply_rotary_pos_emb

    # Forward
    global_out = global_model(input_tensor, pos_embb)[0]
...
def get_argparser():
    parser = argparse.ArgumentParser(
        description="ZigZag‑Mamba2 distributed sanity‑check"
    )

    parser.add_argument(
        "--exp_name", type=str, default="mamba2", help="Experiment name"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to state_dict for loading mamba2 parameters",
    )
    parser.add_argument(
        "--sp",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="SSM state size (a.k.a. per‑head dimension)",
    )
    parser.add_argument(
        "--dtype",
        type=str,
...
def logs_to_json(args, logs, exp_name="mamba2"):
    record = {
    ...
    # One file per run, e.g. results/mamba2_bs1_seq128_bf16.json
