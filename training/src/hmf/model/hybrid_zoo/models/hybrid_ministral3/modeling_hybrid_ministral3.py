...
"""PyTorch Hybrid Ministral3 model.

This module implements a hybrid variant of the Ministral3 architecture that supports replacing standard
...
Architecture:
    This model architecture is based on Ministral3, but allows flexible layer composition through a
    hybrid override pattern specified in the model configuration. Each decoder layer can be either:
    - Standard Ministral3 attention layer (Ministral3DecoderLayer)
    - One of several hybrid layer types (subclasses of Ministral3HybridDecoderLayerBase)
    - Fused attention-hybrid layers for layerwise MSE distillation (subclasses of Ministral3FusedAttHybridDecoderLayer)

Note: Ministral3 does NOT use QK-norm in attention layers (similar to Llama).
"""
...
from .configuration_hybrid_ministral3 import HybridMinistral3Config
...
class Ministral3RotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: HybridMinistral3Config, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: HybridMinistral3Config | None = None,
...
def _get_llama_4_attn_scale(positions_ids: torch.Tensor, beta: float, max_position_embeddings: int) -> torch.Tensor:
    """Llama 4-style attention scaling used by Ministral3."""
...
class Ministral3Attention(nn.Module):
    ...
    def __init__(self, config: HybridMinistral3Config, layer_idx: int):
        super().__init__()
        self.config = config
...
class Ministral3SWAttention(Ministral3Attention):
    """Sliding Window Attention for Ministral3."""
    ...
    def _forward_sp(
        ...
        **kwargs,
        ...
        # Reorder cache_position to match the gathered sequence length.
        # swa_sp_gather transforms [B, L, D] -> [2*B, L//2 + sliding_window, D]
        # via zig-zag split + boundary prepend. cache_position has shape [L], so we
        # reshape it the same way: zig-zag split into two halves, prepend
        # sliding_window positions from the neighbor, producing [L//2 + sliding_window].
        # We unsqueeze to [1, L, 1], run through the same reorder, then squeeze back.
        if cache_position is not None:
            cache_position = swa_sp_reorder_cache_position(
                cache_position, sp_group, self.sliding_window,
            )
        ...
        if sp_rank == 0:
            ...
            if cache_position is not None:
                cache_position = roll_first_half(cache_position.unsqueeze(0), q_len).squeeze(0)
...
class Ministral3HybridDecoderLayerBase(GradientCheckpointingLayer, ABC):
    """
    ...
    @abstractmethod
    def _init_seq_mixer(self, config: HybridMinistral3Config, layer_idx: int):
...
class Ministral3Mamba2DecoderLayer(Ministral3HybridDecoderLayerBase):
    """Decoder layer with Mamba2 as the sequence mixer."""

    def _init_seq_mixer(self, config: HybridMinistral3Config, layer_idx: int):
        # Ministral3 does NOT use QK-norm, so we pass qk_norm=None
...
class Ministral3BMOJOFDecoderLayer(Ministral3HybridDecoderLayerBase):
...
class Ministral3GDNDecoderLayer(Ministral3HybridDecoderLayerBase):
...
class Ministral3GKADecoderLayer(Ministral3HybridDecoderLayerBase):
...
class Ministral3SWADecoderLayer(Ministral3HybridDecoderLayerBase):
    ...
    def _init_seq_mixer(self, config: HybridMinistral3Config, layer_idx: int):
        self.swa = Ministral3SWAttention(config, layer_idx=layer_idx)
...
class Ministral3DualAttDecoderLayer(nn.Module):
    """
...
class Ministral3FusedAttHybridDecoderLayer(nn.Module, ABC):
    """
    ...
    @abstractmethod
    def _init_hybrid_seq_mixer(self, config: HybridMinistral3Config, layer_idx: int):
        """Initializes the sequence mixer (e.g., Mamba2, B'MOJO, etc.)."""
        pass
...
class Ministral3FusedAttMamba2DecoderLayer(Ministral3FusedAttHybridDecoderLayer):
    @property
    def hybrid_seq_mixer(self) -> nn.Module:
        return self.mamba
...
class Ministral3FusedAttBMojoFDecoderLayer(Ministral3FusedAttHybridDecoderLayer):
    @property
    def hybrid_seq_mixer(self) -> nn.Module:
        return self.bmojo_f
...
class Ministral3FusedAttGDNDecoderLayer(Ministral3FusedAttHybridDecoderLayer):
    @property
    def hybrid_seq_mixer(self) -> nn.Module:
        return self.gdn
...
class Ministral3FusedAttGKADecoderLayer(Ministral3FusedAttHybridDecoderLayer):
    @property
    def hybrid_seq_mixer(self) -> nn.Module:
        return self.gka
...
class Ministral3FusedAttSWADecoderLayer(Ministral3FusedAttHybridDecoderLayer):
    @property
    def hybrid_seq_mixer(self) -> nn.Module:
        return self.swa
...
HYBRID_MINISTRAL3_DECODER_LAYER_REGISTRY = build_decoder_layer_registry(
    module_or_dict=sys.modules[__name__],
    model_prefix="Ministral3"
)


@auto_docstring
class HybridMinistral3PreTrainedModel(PreTrainedModel):
    config: HybridMinistral3Config
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = [
        name for name in globals() 
        if name.endswith("DecoderLayer") and "Ministral3" in name
    ]
    ...
    _can_record_outputs = {
        "hidden_states": [Ministral3DecoderLayer, Ministral3HybridDecoderLayerBase],
        "attentions": Ministral3Attention,
    }


@auto_docstring
class HybridMinistral3Model(HybridMinistral3PreTrainedModel):
    def __init__(self, config: HybridMinistral3Config):
    ...
    def _set_decoder_layers(self, config):
        ...
        """
        return set_decoder_layers_from_pattern(config, HYBRID_MINISTRAL3_DECODER_LAYER_REGISTRY)
...
@auto_docstring
class HybridMinistral3ForCausalLM(HybridMinistral3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = HybridMinistral3Model(config)
        self.vocab_size = config.vocab_size
...
class HybridMinistral3ForSequenceClassification(GenericForSequenceClassification, HybridMinistral3PreTrainedModel):
    pass


class HybridMinistral3ForTokenClassification(GenericForTokenClassification, HybridMinistral3PreTrainedModel):
    pass


class HybridMinistral3ForQuestionAnswering(GenericForQuestionAnswering, HybridMinistral3PreTrainedModel):
