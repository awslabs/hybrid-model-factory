...
"""PyTorch Hybrid Qwen2 model.

This module implements a hybrid variant of the Qwen2 architecture that supports replacing standard
...
Architecture:
    This model architecture is based on Qwen2, but allows flexible layer composition through a
    hybrid override pattern specified in the model configuration. Each decoder layer can be either:
    - Standard Qwen2 attention layer (Qwen2DecoderLayer)
    - One of several hybrid layer types (subclasses of Qwen2HybridDecoderLayerBase)
    - Fused attention-hybrid layers for layerwise MSE distillation (subclasses of Qwen2FusedAttHybridDecoderLayer)

Hybrid models can be built as outlined in hybridization/.

Note: Unlike Qwen3, Qwen2 does NOT use QK-norm in attention layers.
"""
...
class Qwen2RotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: HybridQwen2Config, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: HybridQwen2Config | None = None,
        ...
        # Qwen2 computes head_dim from hidden_size and num_attention_heads
...
class Qwen2Attention(nn.Module):
    ...
    Note: Unlike Qwen3, Qwen2 does NOT use QK-norm.
    """

    def __init__(self, config: HybridQwen2Config, layer_idx: int):
        super().__init__()
        ...
        # Qwen2 uses bias=True for Q/K/V projections (unlike Qwen3)
        self.q_proj = nn.Linear(config.hidden_size, config.num_attention_heads * self.head_dim, bias=True)
        self.k_proj = nn.Linear(config.hidden_size, config.num_key_value_heads * self.head_dim, bias=True)
        self.v_proj = nn.Linear(config.hidden_size, config.num_key_value_heads * self.head_dim, bias=True)
        self.o_proj = nn.Linear(config.num_attention_heads * self.head_dim, config.hidden_size, bias=False)
        # Note: Qwen2 does NOT have q_norm and k_norm (unlike Qwen3)
...
class Qwen2SWAttention(Qwen2Attention):
    """Sliding Window Attention variant of Qwen2Attention."""
...
class Qwen2HybridDecoderLayerBase(GradientCheckpointingLayer, ABC):
    """
    ...
    Example:
        class Qwen2CustomDecoderLayer(Qwen2HybridDecoderLayerBase):
    ...
    @abstractmethod
    def _init_seq_mixer(self, config: HybridQwen2Config, layer_idx: int):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Does a forward pass through the Hybrid decoder layer. Similar to Qwen2DecoderLayer's forward
        ...
        Args:
            ...
            output_attentions (`bool`, *optional*):
                ...
                returned tensors for more detail. Ignored for Hybrid Layers.
...
class Qwen2Mamba2DecoderLayer(Qwen2HybridDecoderLayerBase):
    """Decoder layer with Mamba2 as the sequence mixer."""

    def _init_seq_mixer(self, config: HybridQwen2Config, layer_idx: int):
        # Qwen2 does NOT use QK-norm, so we pass qk_norm=None
...
class Qwen2BMOJOFDecoderLayer(Qwen2HybridDecoderLayerBase):
...
class Qwen2GDNDecoderLayer(Qwen2HybridDecoderLayerBase):
...
class Qwen2GKADecoderLayer(Qwen2HybridDecoderLayerBase):
...
class Qwen2SWADecoderLayer(Qwen2HybridDecoderLayerBase):
    ...
    def _init_seq_mixer(self, config: HybridQwen2Config, layer_idx: int):
        self.swa = Qwen2SWAttention(config, layer_idx=layer_idx)
...
class Qwen2DualAttDecoderLayer(nn.Module):
    def __init__(self, config: HybridQwen2Config, layer_idx: int):
        """
    ...
    def _forward(
        ...
        **kwargs,
        ...
        Args:
            ...
            output_attentions (`bool`, *optional*):
                Whether or not to return the attentions tensors of all attention layers. See `attentions` under
                returned tensors for more detail. Not used.
            use_cache (`bool`, *optional*):
                If set to `True`, `past_key_values` key value states are returned and can be used to speed up decoding
                (see `past_key_values`).
            cache_position (`torch.LongTensor` of shape `(sequence_length)`, *optional*):
...
class Qwen2FusedAttHybridDecoderLayer(nn.Module, ABC):
    def __init__(self, config: HybridQwen2Config, layer_idx: int):
        """
        ...
        see Qwen2FusedAttMamba2DecoderLayer).
        """
        super().__init__()
        self.hidden_size = config.hidden_size
        self.attention_type = "full_attention"
        ...
        # Norms below are used by attention and are kept frozen
        self.input_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Qwen2RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
    ...
    @abstractmethod
    def _init_hybrid_seq_mixer(self, config: HybridQwen2Config, layer_idx: int):
        """Initializes the sequence mixer (e.g., Mamba2, B'MOJO, etc.)."""
        pass
...
class Qwen2FusedAttMamba2DecoderLayer(Qwen2FusedAttHybridDecoderLayer):
    @property
    def hybrid_seq_mixer(self) -> nn.Module:
        return self.mamba
...
class Qwen2FusedAttBMojoFDecoderLayer(Qwen2FusedAttHybridDecoderLayer):
    @property
    def hybrid_seq_mixer(self) -> nn.Module:
        return self.bmojo_f
...
class Qwen2FusedAttGDNDecoderLayer(Qwen2FusedAttHybridDecoderLayer):
    @property
    def hybrid_seq_mixer(self) -> nn.Module:
        return self.gdn
...
class Qwen2FusedAttGKADecoderLayer(Qwen2FusedAttHybridDecoderLayer):
    @property
    def hybrid_seq_mixer(self) -> nn.Module:
        return self.gka
...
class Qwen2FusedAttSWADecoderLayer(Qwen2FusedAttHybridDecoderLayer):
    @property
    def hybrid_seq_mixer(self) -> nn.Module:
        return self.swa
...
HYBRID_QWEN2_DECODER_LAYER_REGISTRY = build_decoder_layer_registry(
...
@auto_docstring
class HybridQwen2PreTrainedModel(PreTrainedModel):
    config: HybridQwen2Config
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = [
        name for name in globals() 
        if name.endswith("DecoderLayer") and "Qwen2" in name
    ]
...
@auto_docstring
class HybridQwen2Model(HybridQwen2PreTrainedModel):
    def __init__(self, config: HybridQwen2Config):
    ...
    def _set_decoder_layers(self, config):
        ...
        """
        return set_decoder_layers_from_pattern(config, HYBRID_QWEN2_DECODER_LAYER_REGISTRY)
...
@auto_docstring
class HybridQwen2ForCausalLM(HybridQwen2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = HybridQwen2Model(config)
        self.vocab_size = config.vocab_size
...
class HybridQwen2ForSequenceClassification(GenericForSequenceClassification, HybridQwen2PreTrainedModel):
    pass


class HybridQwen2ForTokenClassification(GenericForTokenClassification, HybridQwen2PreTrainedModel):
    pass


class HybridQwen2ForQuestionAnswering(GenericForQuestionAnswering, HybridQwen2PreTrainedModel):
