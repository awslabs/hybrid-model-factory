...
def apply_liger_kernel_to_hybridqwen3(
    rope: bool = True,
    ...
    """
    Apply Liger kernels to replace original implementation of Hybrid Qwen3 models.
    """
    assert not (cross_entropy and fused_linear_cross_entropy), (
        "cross_entropy and fused_linear_cross_entropy cannot both be True."
    )

    from . import modeling_hybrid_qwen3
    from .modeling_hybrid_qwen3 import HybridQwen3Model

    from liger_kernel.transformers.model.qwen3 import lce_forward as qwen3_lce_forward

    if rope:
        modeling_hybrid_qwen3.apply_rotary_pos_emb = liger_rotary_pos_emb

    if rms_norm:
        modeling_hybrid_qwen3.Qwen3RMSNorm = LigerRMSNorm
    ...
    if fused_linear_cross_entropy:
        if model is not None:
            ...
            modeling_hybrid_qwen3.HybridQwen3ForCausalLM.forward = qwen3_lce_forward

    if swiglu:
        modeling_hybrid_qwen3.Qwen3MLP = LigerSwiGLUMLP
    ...
    if model is not None:
        ...
        base_model: HybridQwen3Model = getattr(model, model.base_model_prefix, model)
