"""
Hybrid Llama Model Construction Module

This module provides functionality for converting base Llama transformer models into hybrid models
...
from hmf.model.hybrid_zoo.models.hybrid_llama.configuration_hybrid_llama import (
    HybridLlamaConfig,
)
...
LLAMA_CLASS_REGISTRY = {
    "ForCausalLM": HybridLlamaForCausalLM,
    "Model": HybridLlamaModel,
    "PreTrainedModel": HybridLlamaPreTrainedModel,
    "ForSequenceClassification": HybridLlamaForSequenceClassification,
    "ForQuestionAnswering": HybridLlamaForQuestionAnswering,
    "ForTokenClassification": HybridLlamaForTokenClassification,
}
...
def get_llama_class(base_model: nn.Module) -> type:
    """
    ...
    for suffix, hybrid_class in LLAMA_CLASS_REGISTRY.items():
        if f"Llama{suffix}" in model_class_name:
            return hybrid_class

    raise ValueError(
        f"Unknown model class: {model_class_name}. "
        f"Expected one of: {[f'Llama{s}' for s in LLAMA_CLASS_REGISTRY.keys()]}"
    )
...
def construct_hybrid_llama(
    base_model: nn.Module,
    config: PretrainedConfig,
    hybrid_config: HybridConfig,
) -> nn.Module:
    """
    Construct a hybrid model from a base Llama model.
    
    This is the main entry point for creating hybrid Llama models. It does the following:
    ...
    Args:
        base_model: Base Llama transformer model to convert
    ...
    """
    logger.section("Constructing Hybrid Llama")

    # Get hybrid model class
    hybrid_llama_class = get_llama_class(base_model)
    logger.info(f"Using hybrid model class: {hybrid_llama_class.__name__}")

    # Create hybrid configuration
    # Pop model_type to avoid overwriting the hybrid config's model_type class attribute if present in base config
    config_dict = config.to_dict()
    config_dict.pop("model_type", None)
    hybrid_llama_config = HybridLlamaConfig.from_dict(config_dict)
    hybrid_llama_config = add_hybrid_configs(
        hybrid_llama_config, hybrid_config=hybrid_config
    )
    ...
    hybrid_model = hybrid_llama_class(config=hybrid_llama_config).to(
    ...
    init_hybrid_layers(
        base_model, hybrid_model, hybridization_candidates, config=hybrid_llama_config
    )
