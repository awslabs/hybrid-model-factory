...
from ..trainer_utils import SaveShardMixin, SequenceParallelBatchSampler, create_custom_optimizer, create_custom_scheduler
...
class CustomSeq2SeqTrainer(SaveShardMixin, Seq2SeqTrainer):
    ...
    def __init__(
        ...
        sequence_parallel_mode: Optional[str] = None,
        **kwargs,
    ) -> None:
    ...
    def compute_loss(self, model, inputs, *args, **kwargs):
        if model.sequence_parallel_group is None:
            ...
            # Validate position_ids for zigzag ring attention
            # position_ids must be monotonically increasing (no resets between samples) for ring attention to work correctly
            # This check only applies to zigzag-ring mode
            pos_ids = inputs.get("position_ids")
            if pos_ids is not None and self.sequence_parallel_mode == "zigzag-ring":
                # Check each half separately, both should be monotonically increasing
                ...
                if not (first_half_valid and second_half_valid):
                    raise ValueError(
                        ...
                        "This usually means the data was tokenized with an older version that "
                        "reset position IDs after every packed sample resetting position_ids. "
                        "Zigzag ring attention requires the tokenized data to have continuous "
