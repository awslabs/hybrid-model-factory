...
class SaveShardMixin:
    """Mixin that adds configurable max_shard_size to _save."""
    ...
    def _save(self, output_dir: str | None = None, state_dict: dict | None = None) -> None:
        ...
        logger.info_rank0(f"Saving model checkpoint to {output_dir}")

        max_shard_size = getattr(self.finetuning_args, "save_max_shard_size", "5GB")
        ...
        if not isinstance(self.model, supported_classes):
            ...
            if isinstance(self.accelerator.unwrap_model(self.model, keep_torch_compile=False), supported_classes):
                self.accelerator.unwrap_model(self.model, keep_torch_compile=False).save_pretrained(
                    output_dir, state_dict=state_dict, max_shard_size=max_shard_size
                )
            else:
            ...
            self.model.save_pretrained(output_dir, state_dict=state_dict, max_shard_size=max_shard_size)
        ...
        if self.processing_class is not None:
            ...
            logger.info_rank0("Saving Trainer.data_collator.tokenizer by default as Trainer.processing_class is `None`")
...
def freeze_params_hybrid(
    ...
    """
    Freezes all model parameters except those specified in finetuning_args.hybrid_learnable_params.

    Arguments:
        model: The model whose parameters we want to freeze.
        trainer: The Hugging Face trainer used to train the model.
        finetuning_args: Parsed finetuning arguments.
        verbose: Whether to print which parameters are learnable as well as
            the total number of learnable parameters. Defaults to True.
    """
    if finetuning_args.hybrid_learnable_params is not None:
        learnable_params = finetuning_args.hybrid_learnable_params.split(",")
        for param_name, param in model.named_parameters():
            if any(learnable_name in param_name for learnable_name in learnable_params):
                param.requires_grad = True
            else:
                param.requires_grad = False

    if verbose and trainer.is_world_process_zero():
        trainable_params = 0
        total_params = 0
        learnable_param_names = []

        for name, param in model.named_parameters():
            # With ZeRO-3, param.numel() returns 0 for partitioned params.
            # Use ds_numel if available, otherwise fall back to numel().
            num_elements = getattr(param, "ds_numel", None) or param.numel()
            total_params += num_elements
            if param.requires_grad:
                trainable_params += num_elements
                learnable_param_names.append(name)

        # Log each learnable parameter
        for name in learnable_param_names:
            logger.info_rank0(f"  {name} is learnable")

        # Log summary
        logger.info_rank0(
            f"Learnable parameters: {trainable_params:,} | Total parameters: {total_params:,} "
            f"({100 * trainable_params / total_params:.2f}%)"
        )


class SequenceParallelBatchSampler(torch.utils.data.Sampler):
    """
    BatchSampler for sequence parallel (SP) training with batch_size > 1.
    
    After tokenizing data with sp_split, the dataset is organized as:
        [seq1_r0, seq1_r1, ..., seq1_r7, seq2_r0, seq2_r1, ..., seq2_r7, ...]
    
    where rX denotes the chunk for SP rank X.
    
    This sampler creates batches such that at each training iteration, all ranks
    process chunks from the same set of sequences, but each rank gets its designated chunk.
    
    Example with sp_size=8 and batch_size=4:
    
        Iteration 0 (all ranks process seq1, seq2, seq3, seq4):
            Rank 0: [seq1_r0, seq2_r0, seq3_r0, seq4_r0]
            Rank 1: [seq1_r1, seq2_r1, seq3_r1, seq4_r1]
            ...
            Rank 7: [seq1_r7, seq2_r7, seq3_r7, seq4_r7]
        
        Iteration 1 (all ranks process seq5, seq6, seq7, seq8):
            Rank 0: [seq5_r0, seq6_r0, seq7_r0, seq8_r0]
            Rank 1: [seq5_r1, seq6_r1, seq7_r1, seq8_r1]
            ...
    
    Args:
        dataset: The dataset to sample from (after sp_split).
        sp_size: The sequence parallel world size.
        batch_size: The micro batch size per device.
    """
    
    def __init__(self, dataset, sp_size: int, batch_size: int):
        self.dataset = dataset
        self.sp_size = sp_size
        self.batch_size = batch_size
        self.num_sequences = len(dataset) // sp_size
        
        # Number of complete batches per SP rank
        self.batches_per_rank = self.num_sequences // batch_size
        # Total number of batches across all ranks
        self.num_batches = self.batches_per_rank * sp_size
    
    def __iter__(self):
        # Generate batches where each batch contains samples from the same SP rank
        # across different sequences
        batches = []
        
        for batch_idx in range(self.batches_per_rank):
            for sp_rank in range(self.sp_size):
                # Collect batch_size samples for this sp_rank
                batch = []
                for i in range(self.batch_size):
                    seq_idx = batch_idx * self.batch_size + i
                    if seq_idx < self.num_sequences:
                        # Index in dataset: seq_idx * sp_size + sp_rank
                        dataset_idx = seq_idx * self.sp_size + sp_rank
                        batch.append(dataset_idx)
