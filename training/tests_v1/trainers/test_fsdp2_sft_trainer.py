...
def test_fsdp2_sft_trainer(tmp_path: Path):
    """Test FSDP2 SFT trainer by simulating `hybridfactory sft config.yaml` behavior."""
    config_yaml = """\
...
"""
    ...
    # This simulates: hybridfactory sft config.yaml
    result = subprocess.run(
        [sys.executable, "-m", "hmf.cli", "sft", str(config_file)],
        env=env,
        capture_output=True,
